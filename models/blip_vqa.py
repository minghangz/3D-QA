...
# print(sys.path)
from models.med import BertConfig, BertModel, BertLMHeadModel, BertEncoder, BertPooler
...
class BLIP_VQA(nn.Module):
    ...
    def forward(self, image, question, answer=None, n=None, weights=None, train=True, inference='rank', k_test=128, image_embeds=None):
        ...
        if train:               
            ...
            if self.selector is not None:
                selector_output = self.selector(
                    inputs_embeds=question_output.last_hidden_state,
                    attention_mask=question.attention_mask,
                    encoder_hidden_states=image_embeds,
                    encoder_attention_mask=image_atts,
                    return_dict=True,
                ...
                # selector_output = self.selector_pooler(selector_output.last_hidden_state)
                loss_pred = self.selector_head(selector_output.pooler_output)
            ...
            # --- selector prediction loss
            if self.selector is not None:
                loss_fct = nn.MSELoss(reduction='none')
                loss_selector = loss_fct(loss_pred, answer_output.loss.detach().clone()).mean()
                loss += self.coeff_selector * loss_selector
            else:
                loss_selector = None

            return loss, loss_selector
